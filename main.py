...
import config
from align import align_phonems, extract_subs
...
def main(sentence, videos, skip=False):
    ...
    transcribed_sentence = get_phonems(sentence)[
        0
    ...
    available_combos = get_best_phonem_combos(
        transcribed_sentence, list(map(lambda x: x.get_phonem(), phonems))
    )
...
def phonemes_from_subs(subs, tmp_folder):
    ...
    for sub in subs:
        ...
        textgrid_path = os.path.join(
            tmp_folder, sub.get_basename_audio() + ".TextGrid"
        )
        phonems.extend(
            parse_align_result(textgrid_path, start_time, sub.get_audio_path())
        )
...
def parse_align_result(path, start_time, full_audio_path):
    ...
    return map(
        lambda x: Phonem(
            x.bounds()[0] + start_time,
            x.bounds()[1] + start_time,
            x.mark,
            full_audio_path,
        ),
        phones,
  ...
  """
    ...
    for url in urls:
        ...
        with youtube_dl.YoutubeDL(ydl_opts) as ydl:
            ...
            paths.append(
                (base_path + ".wav", base_path + ".fr.vtt")
            )  # TODO handle dynamic extensions
    return paths
